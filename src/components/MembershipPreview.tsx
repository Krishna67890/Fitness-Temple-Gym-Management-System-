          ...
          {plans.map((plan, idx) => (
            <motion.div
              ...
              className={`relative glass p-12 rounded-[4rem] flex flex-col transition-all duration-500 group hover:border-primary/40 ${
                plan.popular ? "border-2 border-primary shadow-[0_0_50px_rgba(255,215,0,0.1)] z-10 bg-white/[0.08]" : "border-white/5"
              }`}
            >
              {plan.popular && (
              ...
              <div className="mb-8">
                <h3 className="text-4xl font-black uppercase italic mb-2 tracking-tighter text-white">{plan.name}</h3>
              ...
              <ul className="space-y-6 mb-12 flex-grow">
                {plan.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-center text-gray-300 font-bold text-base tracking-tight group-hover:text-white transition-colors text-left">
